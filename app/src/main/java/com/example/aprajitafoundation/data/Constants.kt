package com.example.aprajitafoundation.data

class Constants {

    companion object {
        const val nameSlide = 1
        const val ImageSlides = 2
        const val EventSlides = 3
        const val phnNumber = "919289228351"
        const val website = "https://theaprajita.in"
        const val apkLink = "https://shorturl.at/B40P1"
        const val email = "theaprajita16@gmail.com"
        const val rpKeyId = "rzp_test_YmqJ4kfwMuj3eh"
        const val logoLink = "https://drive.google.com/uc?export=view&id=1AQen1VF0KyS6UlKZRT6MXCImBaAFgGJ7"

    }
}